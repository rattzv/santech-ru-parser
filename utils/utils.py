...
def update_progress(processed_lines, current_line):
    """
    Обновляет прогресс выполнения процесса, выводя информацию в стандартный вывод.
    Args:
        processed_lines (int): Количество обработанных строк или элементов.
        current_line (str): Текущая строка или элемент.
    Returns:
        None
    """

    sys.stdout.write(print_template(f"№{processed_lines}: current element: {current_line}"))
    sys.stdout.flush()
